...
public class KickPartyCommand extends AbstractAsyncCommand {
   ...
   public KickPartyCommand() {
      ...
      this.usernameArg = this.withRequiredArg("username", "The player to kick", ArgTypes.STRING);
   }
            ...
            return CompletableFuture.runAsync(() -> {
               ...
               if (playerRef != null) {
                  ...
                  if (party == null) {
                     ...
                     NotificationHelper.sendError(playerRef, LanguageManager.getInstance().get("title.party"), LanguageManager.getInstance().get("error.leader_only_kick"));
                  } else {
                     ...
                     if (targetUuid == null) {
                     ...
                     } else if (targetUuid.equals(playerRef.getUuid())) {
                        NotificationHelper.sendError(playerRef, LanguageManager.getInstance().get("title.party"), LanguageManager.getInstance().get("error.cannot_kick_self"));
                        ...
                        manager.markDirty();
                        NotificationHelper.sendSuccess(playerRef, LanguageManager.getInstance().get("title.party"), LanguageManager.getInstance().get("success.player_kicked", targetUsername));
                        NotificationHelper.sendErrorByUuid(targetUuid, LanguageManager.getInstance().get("title.party"), LanguageManager.getInstance().get("notify.you_were_kicked"));
                     }
                  }
               }
            }, world);
